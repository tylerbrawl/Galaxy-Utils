...
class TimeTracker:
    ...
    def get_time_cache(self) -> Dict[str, Dict[str, float]]:
        """
        Returns the current game time cache as a dictionary.

        Example:
            .. code-block:: python

                def update_game_time_cache(self, game_id, context):
                    self.game_time_cache = self.game_time_tracker.get_time_cache()
                    logging.debug(f"Play Time for {game_id}: {self.game_time_cache[game_id]["time_played"]})
                    logging.debug(f"Last Played Time for {game_id}: {self.game_time_cache[game_id]["last_played"]})
                    return self.game_time_tracker.get_tracked_time(game_id)

        """
        if len(self._running_games_dict) > 0:
